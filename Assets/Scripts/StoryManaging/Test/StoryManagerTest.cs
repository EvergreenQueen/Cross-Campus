...
public class StoryManagerTest : MonoBehaviour
{
    ...
    public void Start()
    {
        ...
        returnedValue = StoryManager.Instance.GetContexts(Location.botanical_gardens, "scotty", 0);
        Debug.Log($"test GetContext with botanical gardens, scotty, 0\nexpected: a, results: {FormatContextList(returnedValue)}");
        returnedValue = StoryManager.Instance.GetContexts(Location.botanical_gardens, "scotty", 0);
        Debug.Log($"test GetContext with botanical gardens, scotty, 0\nexpected: a, results: {FormatContextList(returnedValue)}");
        returnedValue = StoryManager.Instance.GetContexts(Location.botanical_gardens, "scotty", 0);
        Debug.Log($"test GetContext with botanical gardens, scotty, 0\nexpected: a, results: {FormatContextList(returnedValue)}");
        
        returnedValue = StoryManager.Instance.GetContexts(Location.botanical_gardens, "scotty", 1);
        Debug.Log($"test GetContext with botanical gardens, scotty, 0\nexpected: a, results: {FormatContextList(returnedValue)}");
        returnedValue = StoryManager.Instance.GetContexts(Location.botanical_gardens, "scotty", 1);
        Debug.Log($"test GetContext with botanical gardens, scotty, 0\nexpected: a, results: {FormatContextList(returnedValue)}");
        returnedValue = StoryManager.Instance.GetContexts(Location.botanical_gardens, "scotty", 1);
        ...
        returnedValue = StoryManager.Instance.GetContexts(Location.classroom, "scotty", 0);
        Debug.Log($"test GetContext with botanical gardens, scotty, 0\nexpected: a, results: {FormatContextList(returnedValue)}");
    }
